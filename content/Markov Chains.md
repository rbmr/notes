...
- The **limiting distribution** is the vector $\pi$ with elements $\pi_{j}=\lim_{ n \to \infty }\mathbb{P}(X_{n}=j)$, $j \in S$, when this limit exists. Whether the limiting distribution exists, and which distribution it is specifically, can depend on the starting distribution on $X_{0}$.
- A probability distribution $\varpi$ is called **stationary** (or **invariant**) if it satisfies the steady-state (balance) equations:
	- $\varpi_j = \sum_{i \in S} \varpi_i P_{ij}$
	- $\sum_{j \in S} \varpi_j = 1$
	- Equivalently, $\varpi = \varpi P$.
- **Every limiting distribution is stationary.** At the limit the distribution is $\pi$, taking one more step gives $\pi P$, but that step must be at the same limit, so $\pi P = \pi$, which is simply the steady-state balance equations.
- **Every stationary distribution is a limiting distribution, for at least one initial distribution.** Starting at $X_0 \sim \varpi$ the chain never leaves $\varpi$, so its limit is $\varpi$.
- If a Markov chain is irreducible and ergodic, then it has exactly one limiting distribution (independent of $X_0$), and a unique stationary distribution $\varpi$ with $\lim_{ n \to \infty } P^n_{ij} = \varpi_j$ from every start $i$.
- A distribution $\varpi$ satisfies the **detailed balance equations** if $\varpi_i P_{ij} = \varpi_j P_{ji}$ for all states $i, j$. A Markov chain for which such a $\varpi$ exists is called **(time-)reversible**: in steady state the flow from $i$ to $j$ matches the flow from $j$ to $i$.
	- **Detailed balance $\Rightarrow$ stationary.** Summing over $i$: $\sum_i \varpi_i P_{ij} = \varpi_j \sum_i P_{ji} = \varpi_j$, the balance equation. So it is a convenient *sufficient* condition for stationarity.
	- **The converse fails.** A stationary distribution need not be reversible — e.g. a biased walk on a ring has uniform stationary distribution but a net circulation, so $\varpi_i P_{i,i+1} \ne \varpi_{i+1} P_{i+1,i}$. Reversibility is strictly stronger than stationarity.
