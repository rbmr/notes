...
- Let $X \subseteq \mathbb{R}^n$ be a convex set, and let $f:X \rightarrow \mathbb{R}$ be differentiable on $X$. Then $f$ is convex if and only if $f(y) \ge f(x) + \nabla f(x)^\top(y-x)$ for all $x,y \in X$.
	- Geometrically: the tangent plane at any point is a global affine lower bound.
