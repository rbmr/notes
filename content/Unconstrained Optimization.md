...
- The analysis of these numerical methods often depends heavily on **Taylor's Theorem** (see [[Multivariate Calculus#Taylor's Theorem]]): for a twice continuously differentiable $f: \mathbb{R}^n \rightarrow \mathbb{R}$ and any $x, p \in \mathbb{R}^n$, there exists a $t \in (0,1)$ such that:
	$$
...
- These two conditions create a delicate tradeoff. The step length must decay slow enough to still allow for fast convergence to the minimizer, but it must decay fast enough to successfully damp the noise once the iterates get close to that minimizer.
	- A common step size schedule is the $p$-sequence (see [[Series#p-series]]): $\alpha_k = c/k^p$ for constant $c$. This adheres to both constraints if $0.5 <p \leq 1$.
...
See [[Subgradients]] for the generalization of the gradient to non-differentiable convex functions.
